...
const App: React.FC = () => {
  ...
  const runTranslation = async (imageSrc: string, scrollY: number, viewHeight: number) => {
    ...
    try {
      // Sadece gerçekten bir kaynak vursa çeviri yap
      if (!imageSrc) return;
      ...
      if (result && result.bubbles && result.bubbles.length > 0) {
        ...
        setBubbles(prev => {
           // Eski balonları belli bir mesafe dışında temizle
           const currentViewMin = scrollY - 4000;
           const currentViewMax = scrollY + 6000;
  ...
  // KRİTİK: Galeri moduna dönüşü zorlayan reset fonksiyonu
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        // İlk çeviriyi anında tetikle
        runTranslation(base64, 0, 1000);
      };
      ...
      <header className={`app-header transition-all duration-500 ease-out ${isHeaderOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="px-6 h-[80px] flex items-center justify-between">
           ...
           <div className="flex items-center space-x-3">
              ...
              {/* GALERI BUTONU - AYNI ZAMANDA RESET BUTONU GOREVI GORUR */}
        ...
        <div className="px-6 pb-6">
           <div className="flex items-center space-x-3 bg-black/50 border border-white/10 rounded-[1.25rem] p-2.5">
              <input 
                ...
                placeholder="Site adresi (örn: mangatrx.com)"
              />
