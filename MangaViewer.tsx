...
const MangaViewer = forwardRef<MangaViewerHandle, MangaViewerProps>(({ bubbles, settings, isProcessing, onScrollStop, customImage, activeUrl }, ref) => {
  ...
  useEffect(() => {
    if (activeUrl) {
      ...
      const timer = setTimeout(() => {
         if (isIframeLoading) {
           console.warn("[MangaViewer] Iframe yüklemesi zaman aşımına uğradı.");
           setIframeError(true);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      timeout = setTimeout(() => {
        // Sadece customImage (base64) varsa çeviri tetiklenir
        if (settings.isEnabled && customImage) {
          onScrollStop(customImage, el.scrollTop, el.clientHeight); 
        }
      }, 1500); 
    };
        ...
        <div ref={containerRef} className="relative w-full flex flex-col min-h-full">
            ...
            {activeUrl ? (
                <div className="w-full relative min-h-screen bg-[#050507]">
                    ...
                    {iframeError ? (
                        <div className="absolute inset-0 z-50 bg-[#050507] flex flex-col items-center justify-center p-10 text-center animate-in zoom-in duration-500">
                            ...
                            <p className="text-zinc-500 text-[11px] mb-10 leading-relaxed font-medium max-w-[280px]">Site güvenlik politikası nedeniyle doğrudan görüntülenemiyor. <br/><br/> <b>ÇÖZÜM:</b> Bölümü tarayıcıda açıp ekran görüntüsü alarak <b>CİHAZDAN YÜKLE</b> özelliğini kullanın.</p>
                ...
                <div className="flex flex-col items-center justify-center min-h-screen p-12 text-center bg-[#050507]">
                    ...
                    <h3 className="text-white font-black text-2xl uppercase tracking-tighter mb-4">MangaTurk AI Hazır</h3>
                    <p className="text-zinc-600 text-[11px] font-bold uppercase tracking-widest leading-loose">Galeriden sayfa seçerek <br/> anlık çeviriye başlayın.</p>
                </div>
            )}
        </div>
      </div>
